...
class Physics:
    ...
    def update(self, dt: float, gravity_direction: int = 1) -> None: 
        ...
        self.velocity = Vector2(
            self.velocity.x,
            self.velocity.y - self.gravity * gravity_direction * dt  
        )
        ...
        if (self.position.y < -600) or (self.position.y > 1100):
    ...
    def resolve_collision(self, other: 'Physics', side: str, overlap: float) -> None:
        ...
        if side == 'top':
            ...
            self.position = Vector2(
                other_left - self.width / 2 - 0.1, 
                self.position.y
            )

        elif side == 'right':
            self.position = Vector2(
                other_right + self.width / 2 + 0.1,
                self.position.y
            )
            ...
            self.position = Vector2(
                self.position.x,
                other_bottom - self.height / 2 - 0.1 
            )
            if self.velocity.y > 0:
                self.velocity = Vector2(self.velocity.x, 0)
