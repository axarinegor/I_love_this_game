...
class Level_10:
    def __init__(self,
        ...
        self.chain_texture = Lev_Patterns.get_default_chain()
        self.blocks_chains = [
        ...
        self.areas = [
            {"x": 0, "blocks": []},  # Левая
            {"x": SHAPE.x // 3, "blocks": []},  # Средняя
            {"x": 2 * SHAPE.x // 3, "blocks": []},  # Правая
        ]
        self.blocks_spawned = [False, False, False]
    ...
    def on_mouse_press(self, x: int, y: int, button: int, modifiers: int) -> None:
        if button == arcade.MOUSE_BUTTON_MIDDLE:
            for i, area in enumerate(self.areas):
                ...
                if left <= x <= right:
                    if not self.blocks_spawned[i]:
                        area = self.areas[i]
                        area["blocks"].append(self.blocks_chains[i])
                        self.blocks_spawned[i] = True
    ...
    def on_draw(self) -> None:
        ...
        for area in self.areas:
            for block in area["blocks"]:
                ...
                self._draw.chain(block, self.chain_texture)
