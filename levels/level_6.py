...
class Level_6:
    ...
    def on_fixed_update(self, delta_time: float) -> None:
        if not self._door.is_open:
            all_collision_objects = self._platforms + [self._door] + self.created_blocks
            self._player.update(delta_time, all_collision_objects)
            return
        self._player.update(delta_time, self._platforms + self.created_blocks)
    ...
    def on_mouse_motion(self, x: int, y: int, dx: int, dy: int) -> None:
        if self.is_drawing:
            ...
            if (current_pos - self.last_mouse_pos).length > self.block_size:
                self._create_block(x, y)
...
class Level_6:
    ...
    def on_fixed_update(self, delta_time: float) -> None:
        if not self._door.is_open:
            all_collision_objects = self._platforms + [self._door] + self._created_blocks
            self._player.update(delta_time, all_collision_objects)
            return
        self._player.update(delta_time, self._platforms + self._created_blocks)
        if GameRules.check_level_completion(self._player, self._exit_position):
    ...
    def on_mouse_press(self, x: int, y: int, button: int, modifiers: int) -> None:
        if button == arcade.MOUSE_BUTTON_LEFT:
            ...
            self._last_mouse_pos = Vector2(x, y)
            self._create_block(x, y)
    ...
    def on_mouse_motion(self, x: int, y: int, dx: int, dy: int) -> None:
        if self._is_drawing:
            current_pos = Vector2(x, y)
            if (current_pos - self._last_mouse_pos).length > self._block_size:
                self._create_block(x, y)
                self._last_mouse_pos = current_pos

    def _create_block(self, x: int, y: int) -> None:
        block = Platform(_physics=Physics(
            _position=Vector2(x, y),
            _width=self._block_size,
            _height=self._block_size,
            _is_active=True)
        )
    ...
    def on_draw(self) -> None:
        ...
        for block in self._created_blocks:
            self._draw.platform(block)
        self._draw.door(self._door, Lev_Patterns.get_default_door())
        self._draw.door(self._left_door, Lev_Patterns.get_default_door())
        self._draw.texts(LEVEL_6)