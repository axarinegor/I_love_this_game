...
from texts import *
from level_pattern import Lev_Patterns, blocks_19_1, blocks_19_2
...
class Level_19:
    ...
    def on_fixed_update(self, delta_time: float) -> None:
        if not self._door.is_open:
            all_collision_objects = self._platforms + [self._door] + self.current_blocks
            self._player.update(delta_time, all_collision_objects)
            return
        self._player.update(delta_time, self._platforms + self.current_blocks)
...
class Level_19:
    def __init__(self,
        ...
        self._current_blocks = blocks_19_1
        self._is_first_set = True
    ...
    def on_fixed_update(self, delta_time: float) -> None:
        if not self._door.is_open:
            all_collision_objects = self._platforms + [self._door] + self._current_blocks
            self._player.update(delta_time, all_collision_objects)
            return
        self._player.update(delta_time, self._platforms + self._current_blocks)
        if GameRules.check_level_completion(self._player, self._exit_position):
            GameRules.complete_level(self._game_app, self._level_num)

    def on_mouse_press(self, x: int, y: int, button: int, modifiers: int) -> None:
        if button == arcade.MOUSE_BUTTON_LEFT:
            if self._is_first_set:
                self._current_blocks = blocks_19_2
            else:
                self._current_blocks = blocks_19_1
    ...
    def on_draw(self) -> None:
        ...
        for platform in self._current_blocks:
            self._draw.platform(platform)
            self._draw.texture_wall(platform, self._block_texture)
        self._draw.door(self._door, Lev_Patterns.get_default_door())
        self._draw.door(self._left_door, Lev_Patterns.get_default_door())
