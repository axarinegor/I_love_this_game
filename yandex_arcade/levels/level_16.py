...
class Level_16:
    def __init__(self,
        ...
        self._countdown_timer = 5.0
        self._countdown_active = True
        self._memory_blocks = [
            Physics(_position=Vector2(300, 150), _width=BLOCK_HEIGHT, _height=BLOCK_HEIGHT),
            Physics(_position=Vector2(420, 290), _width=BLOCK_HEIGHT, _height=BLOCK_HEIGHT),
            Physics(_position=Vector2(700, 140), _width=BLOCK_HEIGHT, _height=BLOCK_HEIGHT),
        ]
    ...
    def on_fixed_update(self, delta_time: float) -> None:
        if self._countdown_active:
            ...
            self._timer_text.text = f"{max(0, int(self._countdown_timer))}"
            return
        
        if not self._door.is_open:
            all_collision_objects = self._platforms + [self._door] + self._memory_blocks
            self._player.update(delta_time, all_collision_objects)
            return
        self._player.update(delta_time, self._platforms + self._memory_blocks)
