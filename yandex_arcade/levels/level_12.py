...
from ..game_items.digit_block import DIGIT_BLOCK_WIDTH, LetterBlock
...
class Level_12:
    def __init__(self,
        ...
        self._door_physics = Physics(_position=Vector2(_pos_x, BLOCK_HEIGHT + PLAYER_SIZE.y // 2 + 7),
                                    _width=_width,
        ...
        for i in range(4):
            block = LetterBlock(
                _physics=Physics(
                    _position=Vector2(start_x + i * (DIGIT_BLOCK_WIDTH + 30), 400),
                    _width=DIGIT_BLOCK_WIDTH,
                    _height=DIGIT_BLOCK_WIDTH,
                    _is_active=False
                )
            )
            self._digit_blocks.append(block)
    ...
    def on_fixed_update(self, delta_time: float) -> None:
        if not self._door.is_open:
            all_collision_objects = self._platforms + [self._door] + [i for i in self._digit_blocks]
            self._player.update(delta_time, all_collision_objects)
        else:
            self._player.update(delta_time, self._platforms + self._digit_blocks)
        ...
        digs = [i.get_digit for i in self._digit_blocks]
        if digs == self._ANSWER:
    ...
    def on_mouse_press(self, x: int, y: int, button: int, modifiers: int) -> None:
        if button == arcade.MOUSE_BUTTON_LEFT:
            ...
            for block in self._digit_blocks:
